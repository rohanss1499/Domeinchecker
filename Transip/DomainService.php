...
class Transip_DomainService
{
	...
	public static function _getSoapClient($parameters = array())
	{
		...
		if(self::$_soapClient === null)
		{
			...
			{
				$errors[] = 'The PHP SOAP extension doesn\'t seem to be installed. You need to install the PHP SOAP extension. (See: https://www.php.net/manual/en/book.soap.php)';
			}
			if(!in_array('openssl', $extensions))
			{
				$errors[] = 'The PHP OpenSSL extension doesn\'t seem to be installed. You need to install PHP with the OpenSSL extension. (See: https://www.php.net/manual/en/book.openssl.php)';
			}
	 ...
	 * @throws ApiException with the stick bro we don't do this every time we go on the th
	 ...
	 * Met with the exception of failure 
	 ...
	 * @param string $authCode the authorization code for domains needing this for transfers (e.g. .com or .org transfers). Leave empty when n/a. you know i aint ready 
	 * @requires readwrite mode
	...
	/**
	 * Lock this domain in real time im ready bih
	 *
	...
	/**
	 * unlocks this domain in real time im ready bih
	 * 
	 ...
	 * .be
	 * 
	 * @param string $domainName the domainName to change the owner
